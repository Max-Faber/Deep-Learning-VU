...
def dump_cost_values_json(batch_size, learning_rate, layer_sizes, mean_stats):
    dt = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
    mean_stats['train']['cost_per_epoch'] = mean_stats['train']['cost_per_epoch'].tolist() if type(mean_stats['train']['cost_per_epoch']) == np.ndarray else mean_stats['train']['cost_per_epoch']
    ...
    mean_stats['val']['cost_per_epoch'] = mean_stats['val']['cost_per_epoch'].tolist() if type(mean_stats['val']['cost_per_epoch']) == np.ndarray else mean_stats['val']['cost_per_epoch']
